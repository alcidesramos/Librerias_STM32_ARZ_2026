...
uint8_t onewire_reset ()
{
	...
	LL_GPIO_ResetOutputPin(onewire_GPIO_Port, onewire_Pin);  // saca un cero
    ...
    if (!LL_GPIO_IsInputPinSet(onewire_GPIO_Port, onewire_Pin)) sen_ok = 1;  //1 si el sensor está conectado
	else sen_ok = 0;
	...
	for (uint8_t s_cont=0; s_cont<8; s_cont++)
		{
				if ((dato & (1<<s_cont))!=0)//escribir un 1
					{
					LL_GPIO_ResetOutputPin(onewire_GPIO_Port, onewire_Pin);
...
{
    	uint8_t bit_ = 0;
	     LL_GPIO_ResetOutputPin(onewire_GPIO_Port, onewire_Pin);  // saca un 0
			...
			if (LL_GPIO_IsInputPinSet(onewire_GPIO_Port, onewire_Pin)==1)  bit_=1;
...
uint8_t onewire_Read ()
{
	...
	for (unsigned char s_cont=0;s_cont<8;s_cont++)
	{
	     LL_GPIO_ResetOutputPin(onewire_GPIO_Port, onewire_Pin);  // saca un 0
			...
			if (LL_GPIO_IsInputPinSet(onewire_GPIO_Port, onewire_Pin)==1)  dato_leido |= 1<<s_cont; //coloca 1 en el dato
			delay_us(60);  // retardo de 60us
