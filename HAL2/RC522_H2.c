...
static void MFRC522_Wr( uint8_t addr, uint8_t value )
{
     uint8_t dato;
     dato=(( addr << 1 ) & 0x7E);
	 HAL_GPIO_WritePin(RC522_CS_PORT, RC522_CS_PIN, 0);
     HAL_SPI_Transmit(&hspi1, &dato, 1, 100);
     HAL_SPI_Transmit(&hspi1, &value, 1, 100);
  
     HAL_GPIO_WritePin(RC522_CS_PORT, RC522_CS_PIN, 1);
}
...
void MFRC522_Init()
{
   delay_init();
     ...
     //ACCION DE RESET
     HAL_GPIO_WritePin(RC522_RST_PORT, RC522_RST_PIN, 0);
     Delay_us(2);   //TIEMPO PARA COMPLETAR RESET
	 HAL_GPIO_WritePin(RC522_RST_PORT, RC522_RST_PIN, 1);//LISTO PARA INICIAR
     HAL_Delay(50);
