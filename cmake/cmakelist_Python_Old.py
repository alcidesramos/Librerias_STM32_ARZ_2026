...
class CMakeGeneratorApp:
    def __init__(self, root):
        ...
        # --- MEJORA: Ventana más ancha de inicio y redimensionable a lo ancho ---
        self.root.geometry("680x440")
        self.root.minsize(580, 440)  # Límite mínimo para que no se colapse
    ...
    def crear_interfaz(self):
        ...
        # Optimización dinámica (MEJORADA PARA EXPANDIRSE)
        ...
        # fill="x" y expand=True hacen que el Combobox crezca con la ventana
    ...
    def toggle_opt_combobox(self):
        if self.var_opt.get():
            self.combo_opt.config(state="readonly")
        else:
            self.combo_opt.config(state="disabled")
    ...
    def procesar_cmakelist(self):
        ...
        if not (self.var_lib.get() or self.var_float.get() or self.var_hex.get() or self.var_opt.get()):
        ...
        if self.var_lib.get():
            ...
            bloques_no_seleccionados.append("Librerías automáticas")
        ...
        if self.var_float.get():
            ...
            bloques_no_seleccionados.append("Soporte float/math (-lm)")
        ...
        if self.var_hex.get():
            ...
            bloques_no_seleccionados.append("Generación HEX/BIN")
        ...
        if self.var_opt.get():
            ...
            bloques_no_seleccionados.append("Optimización de Release")

        if not bloques_a_escribir:
            messagebox.showinfo(
                "Nada que hacer",
                "Todas las opciones seleccionadas ya estaban agregadas en el CMakeLists.txt.\n"
        ...
        try:
            ...
            nombres_agregados = []
            if self.var_lib.get() and MARCADORES["lib"] not in contenido_actual:
                nombres_agregados.append("Librerías automáticas")
            if self.var_float.get() and MARCADORES["float"] not in contenido_actual:
                nombres_agregados.append("Soporte float/math (-lm)")
            if self.var_hex.get() and MARCADORES["hex"] not in contenido_actual:
                nombres_agregados.append("Generación HEX/BIN")
            if self.var_opt.get() and MARCADORES["opt"] not in contenido_actual:
                nombres_agregados.append("Optimización de Release")

            mensaje = f"Configuraciones añadidas a:\n{self.cmake_file_path}\n\n"
