...
extern I2C_HandleTypeDef hi2c1;//puerto i2c usado
#define i2c_max30102 hi2c1

extern UART_HandleTypeDef huart1;//puerto serial a usar
#define  uart_max30102 huart1
char text[30];
...
void I2C_Init(void)
{
	...
	if (HAL_I2C_Init(&i2c_max30102) != HAL_OK)
	{
		errorHandler();
	}
}
...
int8_t pulseOximeter_readRegister(uint8_t reg, uint8_t* value)
{
	...
	retStatus = HAL_I2C_Master_Transmit(&i2c_max30102, address, buf, 1, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}

	address = (I2C_SLAVE_ID | I2C_READ);
	retStatus = HAL_I2C_Master_Receive(&i2c_max30102, address, buf, 1, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}
...
HAL_StatusTypeDef pulseOximeter_writeRegister(uint8_t reg, uint8_t value)
{
	...
	uint8_t address = (I2C_SLAVE_ID | I2C_WRITE);
	retStatus = HAL_I2C_Master_Transmit(&i2c_max30102, address, buf, 2, HAL_MAX_DELAY);
...
FIFO_LED_DATA pulseOximeter_readFifo(void)
{
	...
	HAL_I2C_Master_Transmit(&i2c_max30102, address, buf, 1, HAL_MAX_DELAY);

	address = (I2C_SLAVE_ID | I2C_READ);
	HAL_I2C_Master_Receive(&i2c_max30102, address, buf, numBytes, HAL_MAX_DELAY);
...
int8_t pulseOximeter_readFIFO(uint8_t* dataBuf, uint8_t numBytes)
{
	...
	address = (I2C_SLAVE_ID | I2C_READ);
	retStatus = HAL_I2C_Master_Receive(&i2c_max30102, address, buf, numBytes, HAL_MAX_DELAY);
	if( retStatus != HAL_OK ){
		return -1;
	}
...
void pulseOximiter_registerDump(void)
{
	...
	if( readStatus == -1){
		uartx_write_text(&uart_max30102, "I2C read error!");
		return;
	}

	//itoa(readResult, buf, 16);   // here 16 means Hexadecimal
     uartx_write_text(&uart_max30102, "INT_STATUS_1: 0x");
  sprintf(text,"%d",readResult);
  uartx_write_text(&uart_max30102, text);
	...
	//itoa(readResult, buf, 16);   // here 16 means Hexadecimal
    uartx_write_text(&uart_max30102, "INT_STATUS_2: 0x");
    sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	//itoa(readResult, buf, 16);   // here 16 means Hexadecimal
     uartx_write_text(&uart_max30102, "INT_ENABLE_1: 0x");
     sprintf(text,"%d",readResult);
       uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "INT_ENABLE_2: 0x");
	sprintf(text,"%d",readResult);
	  uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "FIFO_WRITE_PTR: 0x");
	sprintf(text,"%d",readResult);
	  uartx_write_text(&uart_max30102, text);
    ...
    uartx_write_text(&uart_max30102, "FIFO_OVF_COUNTER: 0x");
	 sprintf(text,"%d",readResult);
	  uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "FIFO_READ_POINTER: 0x");
	sprintf(text,"%d",readResult);
	uartx_write_text(&uart_max30102, text);
   ...
   uartx_write_text(&uart_max30102, "I2C read error!");
	...
	uartx_write_text(&uart_max30102, "FIFO_DATA_REGISTER: 0x");
	sprintf(text,"%d",readResult);
	uartx_write_text(&uart_max30102, text);
    ...
    uartx_write_text(&uart_max30102, "FIFO_CONFIG: 0x");
    sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "MODE_CONFIG: 0x");
	sprintf(text,"%d",readResult);
   uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "SPO2_CONFIG: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "LED_PULSE_AMP_1: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
    ...
    uartx_write_text(&uart_max30102, "LED_PULSE_AMP_2: 0x");
    sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "LED_MODE_CONTROL_1: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "LED_MODE_CONTROL_2: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "DIE_TEMP_INTEGER: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "DIE_TEMP_FRACTION: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "DIE_TEMP_CONFIG: 0x");
	sprintf(text,"%d",readResult);
    uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "REVISION_ID: 0x");
	sprintf(text,"%d",readResult);
	uartx_write_text(&uart_max30102, text);
	...
	uartx_write_text(&uart_max30102, "PART_ID: 0x");
	sprintf(text,"%d",readResult);
	uartx_write_text(&uart_max30102, text);


	uartx_write_text(&uart_max30102, "\r\n");
}

void pulseOximeter_displayData(void)
{

	//uartx_write_text(&uart_max30102, "MAX30102 Pulse Oximiter Data:\r\n");

	uartx_write_text(&uart_max30102, "Temperatura= ");
	sprintf(text,"%.2f\r\n",pulseOximeter.temperature);
	uartx_write_text(&uart_max30102, text);
	//uart_PrintFloat(pulseOximeter.temperature);

	uartx_write_text(&uart_max30102, "Pulso(BPM)=");
	sprintf(text,"%.2f\r\n",pulseOximeter.heartBPM);
   uartx_write_text(&uart_max30102, text);
//uart_PrintFloat(pulseOximeter.heartBPM);

	uartx_write_text(&uart_max30102, "Saturatio(%)= ");
	sprintf(text,"%.2f\r\n",pulseOximeter.SpO2);
   uartx_write_text(&uart_max30102, text);
