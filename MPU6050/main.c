...
if (MPU6050_init()==1)  uartx_write_text(&huart1, "SENSOR OKS\r\n");
  else  uartx_write_text(&huart1, "BAD \r\n");
...
*/
 ...
 MPU6050_Calibra();
 /* sprintf(texto, "%d %d %d\r\n\r\n",(int32_t)off_giro_x,(int32_t)off_giro_y,(int32_t)off_giro_z);
  uartx_write_text(&huart1, texto);

MPU6050_lee_datos();
sprintf(texto, "%d %d %d\r\n\r\n",(int16_t)MPU6050.giro_x,(int16_t)MPU6050.giro_y,(int16_t)MPU6050.giro_z);
uartx_write_text(&huart1, texto);
while (1);
*/
 //TIMERIMU_Init();
 ...
 /* while (1)
  {
	  GY85_Ace_angulos();
	GY85_Read_bruj();
sprintf(texto, "D1=%u    %u   %u\r\n\r\n",imu.bruj_x,imu.bruj_y,imu.bruj_z);
uartx_write_text(&huart1, texto);
HAL_Delay(1000);
	//	Bruj_heading();
